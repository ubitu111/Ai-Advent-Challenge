...
18. **build_release_apk** - Собрать подписанный релизный APK из проекта Git репозитория, используя Gradle команды
    - Параметры: нет
    - Требует настройки локального Git (см. раздел "Настройка локального Git")
    - Собранный APK сохраняется в папку `builds` в директории сервера с датой и временем в названии (формат: `app-release_YYYYMMDD_HHMMSS.apk`)
    - Возвращает путь к собранному APK файлу
    - Пример использования см. в разделе "Сборка APK"

19. **upload_apk_to_yandex_disk** - Загрузить APK файл на Яндекс Диск и получить публичную ссылку
    - Параметры: `apk_path` (string, обязательный) - путь к APK файлу для загрузки
    - Требует настройки Яндекс Диск API (см. раздел "Настройка Яндекс Диск API")
    - Возвращает публичную ссылку на загруженный файл
    - Пример: `{"apk_path": "/path/to/app-release.apk"}` - загрузить APK файл
    - Пример использования см. в разделе "Загрузка APK на Яндекс Диск"

## Сборка APK

Инструмент `build_release_apk` позволяет собрать подписанный релизный APK из Android проекта, используя Gradle команды.

### Требования

- Локальный Git репозиторий должен быть настроен (см. раздел "Настройка локального Git")
- В репозитории должен быть Android проект с настроенным Gradle
- Должен быть доступен Gradle wrapper (`gradlew` или `gradlew.bat`)
- Проект должен быть настроен для сборки релизного APK (подписанный APK)

### Как это работает

1. Инструмент использует путь к Git репозиторию из конфигурации `GIT_REPO_PATH`
2. Выполняется команда `./gradlew assembleRelease` (или `gradlew.bat assembleRelease` на Windows)
3. После успешной сборки APK файл ищется в стандартных местах:
   - `androidApp/build/outputs/apk/release/`
   - `app/build/outputs/apk/release/`
   - `build/outputs/apk/release/`
   - `android/build/outputs/apk/release/`
4. Найденный APK копируется в папку `builds` в директории сервера
5. В название файла добавляется дата и время сборки (формат: `app-release_YYYYMMDD_HHMMSS.apk`)
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "APK успешно собран!\nПуть к файлу: /path/to/server/builds/app-release_20241222_143025.apk\n\nAPK сохранен в папку builds с датой и временем в названии."
      }
    ],
    "isError": false
  }
...
## Загрузка APK на Яндекс Диск

Инструмент `upload_apk_to_yandex_disk` позволяет загрузить APK файл на Яндекс Диск и получить публичную ссылку для скачивания.

### Настройка Яндекс Диск API

Для использования инструмента необходимо настроить OAuth токен Яндекс Диска.

#### Шаг 1: Создание приложения в Яндекс ID

1. Перейдите на страницу разработчика: https://oauth.yandex.ru/
2. Нажмите "Зарегистрировать новое приложение"
3. Заполните форму:
   - **Название приложения**: например, "MCP Server"
   - **Платформы**: выберите "Веб-сервисы"
   - **Callback URI**: можно указать `http://localhost:8080` (для локальной разработки)
   - **Права доступа**: выберите "cloud_api:disk.app_folder" или "cloud_api:disk.write" для записи файлов
...
Есть два способа получить OAuth токен:

##### Способ 1: Через браузер (для тестирования)

1. Откройте в браузере следующую ссылку (замените `YOUR_CLIENT_ID` на ваш Client ID):
   ```
   https://oauth.yandex.ru/authorize?response_type=token&client_id=YOUR_CLIENT_ID
   ```
2. Войдите в свой аккаунт Яндекс
3. Разрешите доступ приложению
4. После перенаправления в адресной строке браузера будет токен в формате `access_token=YOUR_TOKEN`
5. Скопируйте токен (часть после `access_token=`)

##### Способ 2: Через API (для production)

Используйте OAuth 2.0 flow для получения токена программно. Подробнее: https://yandex.ru/dev/id/doc/ru/

#### Шаг 3: Настройка переменных окружения

Установите переменную окружения с OAuth токеном:
...
**Windows (PowerShell):**
```powershell
$env:YANDEX_DISK_TOKEN="YOUR_OAUTH_TOKEN"
```

**Windows (CMD):**
```cmd
set YANDEX_DISK_TOKEN=YOUR_OAUTH_TOKEN
```

#### Альтернативный способ: Системные свойства

Запустите сервер с параметром:

```bash
./gradlew :server:run -Dyandex.disk.token=YOUR_OAUTH_TOKEN
```
...
|----------|----------|--------|
| `YANDEX_DISK_TOKEN` | OAuth токен для доступа к Яндекс Диск API | `AQAAAAAxxxxxxxxxxxxxxxxxxxx` |
...
Если вы получаете ошибку `401 Unauthorized`, проверьте следующее:

1. **Токен правильный и не истек**
   - Токены могут истекать или быть отозваны
   - Проверьте токен на странице управления доступом: https://yandex.ru/id/security
   - При необходимости получите новый токен

2. **Токен имеет нужные права**
   - Убедитесь, что при создании приложения вы выбрали права `cloud_api:disk.write` или `cloud_api:disk.app_folder`
   - Проверьте права приложения в консоли разработчика: https://oauth.yandex.ru/

3. **Токен не содержит лишних символов**
   - Токен не должен содержать пробелов в начале или конце
   - Токен не должен содержать переносов строк
   - Убедитесь, что при копировании токена вы скопировали его полностью
   - Сервер автоматически очищает токен от пробелов и переносов строк

4. **Проверка токена**
   - Сервер автоматически проверяет токен перед использованием
   - В логах вы увидите сообщения о проблемах с токеном
   - Проверьте логи сервера для получения детальной информации об ошибке
   - Логи показывают длину токена и первые символы (для отладки, без раскрытия полного токена)

5. **Формат токена**
   - Токен должен быть в формате OAuth токена Яндекс ID
   - Обычно токен выглядит как длинная строка символов (например: `AQAAAAAxxxxxxxxxxxxxxxxxxxx`)
   - Не добавляйте префиксы типа "Bearer" или "OAuth" - они добавляются автоматически

**Пример правильной настройки токена:**

```bash
# Правильно - токен без пробелов и переносов строк
export YANDEX_DISK_TOKEN=AQAAAAAxxxxxxxxxxxxxxxxxxxx

# Неправильно - токен с пробелами
export YANDEX_DISK_TOKEN=" AQAAAAAxxxxxxxxxxxxxxxxxxxx "

# Неправильно - токен с переносами строк
export YANDEX_DISK_TOKEN="AQAAAAAxxxxxxxxx
xxxxx"
```

**Проверка токена вручную:**

Вы можете проверить токен, сделав запрос к API Яндекс Диска:

```bash
curl -H "Authorization: OAuth YOUR_TOKEN" https://cloud-api.yandex.net/v1/disk
```

Если токен правильный, вы получите информацию о диске. Если нет - ошибку 401.

### Примеры использования

**Загрузить APK на Яндекс Диск:**
```bash
curl -X POST http://localhost:8080/mcp \
  -H "Content-Type: application/json" \
  -d '{
    ...
    "method": "tools/call",
    "params": {
      "name": "upload_apk_to_yandex_disk",
      "arguments": {
        "apk_path": "/path/to/server/builds/app-release_20241222_143025.apk"
      }
    }
  }'
```
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "APK успешно загружен на Яндекс Диск!\nПубличная ссылка: https://disk.yandex.ru/d/xxxxxxxxxxxxxxxxxxxx\n\nФайл доступен по указанной ссылке для скачивания."
      }
    ],
    "isError": false
  }
...
**Полный пример: сборка и загрузка APK:**
```bash
# Шаг 1: Собрать APK
BUILD_RESPONSE=$(curl -X POST http://localhost:8080/mcp \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
    "id": 22,
...
# Извлечь путь к APK из ответа (требует парсинга JSON)
# Затем загрузить на Яндекс Диск
curl -X POST http://localhost:8080/mcp \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
